...
`pet.json` 매니페스트로 패키지 형식을 확인할 수 있다. 스프라이트시트는 없으니
[직접 그리기](#직접-그리기)로 간다.
...
펫을 우클릭해 **펫 바꾸기**를 고르면 그 값이 `config.json`에 남는다.
...
두 파일이 든 폴더를 넣기만 하면 메뉴에 뜨고, 타이밍 표와 `lines.json`은 모든 펫이 같이
쓰니 코드를 고칠 일도 펫마다 따로 맞출 일도 없다.
