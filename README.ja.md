...
**Claude Code** と **Codex** が今なにをしているかを表示するデスクトップペット。

[English](README.md) · [한국어](README.ko.md) · **日本語** · [简体中文](README.zh-CN.md)

</div>

デスクトップに小さなキャラクターが住み、コーディングエージェントが今なにをしているかを
表示する。**Claude Code** と **Codex** の両方を見る。

エージェントが作業を始めればペットも働き、返事を待っていればこちらを向いて止まる。
失敗すれば赤くなるので、ウィンドウを切り替えなくても気づく。

ウィンドウは透過で常に最前面にあり、作業中の画面を隠さない。Windows 用の
**PySide6**（Qt）アプリで、状態を記録するフックは WSL で動く。
...
Claude Code はフックを登録する必要があり、Codex は設定が要らない。Codex が自分で残して
いるログをオーバーレイが直接読むからだ。オーバーレイはそのファイルを定期的に確認して、
見つけた状態のアニメーションを再生する。
...
マウスが近づくと目を向ける。カーソルが入ったとき、クリックしたとき、状態が変わったときに
...
クリック時の動作、自律行動のオンオフ、セリフと音声、音楽フォルダ、作業表示の設定がそこにある。
...
同梱の `yui` CLI が同じ `PetState` を書く。長く走るジョブを包めば、ペットがその状態を表示する。
...
ペットを右クリックして **ペットを変える**を選ぶ。選んだ値は `config.json` に残る。
...
ので、自分のキャラクターを描くか、権利を持っているシートをローダーに渡せばいい。
