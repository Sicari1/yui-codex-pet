...
開いているウィンドウの上辺に登ってその上を歩き、画面の壁をよじ登る。つかんで引けば引いた
方向へ走り、動かしたまま放せば本物の弧を描いて飛び、画面の端で跳ね返って落ち着く。

**見てはくるが、ずっとではない。** 16 方向の視線を持つが、カーソルが近づいた・クリックした・
状態が変わった、そのときだけ 3〜5 秒目を向ける。入力中はこちらから見つめない。見続ければ
それは凝視になるからだ。

**頼めば。** 右クリックメニューには手を振る・ジャンプ・壁を登る・ポモドーロ開始（25 分集中・
5 分休憩）・ペットの切り替え・音楽プレイヤーが並ぶ。残りは設定ウィンドウに集約した — 言語、
サイズ、透明度、クリック時の動作、自律行動のオンオフ、セリフと音声、音楽フォルダ、作業表示の設定。
タスクトレイにも常駐し、クリック透過を戻せる経路はそのトレイだけだ。

**音楽。** 自分のフォルダの音源をプレイヤーウィンドウで再生する。検索・シャッフルと、
歌・インスト・BGM のフィルタがあり、ペットが喋る間は音量が一時的に下がる。

**三つの言語。** 韓国語・English・日本語。画面の文言だけでなくペットのセリフまで切り替わり、
再起動なしでその場で反映される。
