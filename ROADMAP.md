...
Last Updated: 2026-07-25 (음성 설계 2-D 추가)

Claude Code 상태 표시기에서 **평소에 데리고 사는 데스크톱 펫**으로 넓히기 위한 아이디어 모음.
지금 구현을 기준으로 **당장 되는 것**과 **스프라이트가 나와야 되는 것**을 갈라 놓았다.
스프라이트 재제작(→ `STATE.md` 1순위)이 여러 항목의 전제라, 그릴 목록을 여기서 확정한다.

---

## 0. 설계 원칙

데스크톱 펫이 지워지는 이유는 대개 **귀엽지 않아서가 아니라 거슬려서**다. 그래서 순서를 이렇게 둔다.
...
`qa/direction-semantics.json`은 이걸 전부 통과시켰다(4방위 pass, 중간 각도는 "blind 검증에서
단서가 미묘함" warning, `"ok": true`). 그런데 정작 오버레이에는 13·14·15를 스냅하는
